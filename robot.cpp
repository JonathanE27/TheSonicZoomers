    ...
    double red [size];
	  ...
	  for(int i = 0; i <150; i++){
	   ...
	   // std::cout<<isBlack<<" ";
		...
		r = std::distance(red, std::find(values, red + size, 1));	//cout<<R<<std::endl; //find white values
			//Print out isWhite 
			//std::cout<<isWhite<<" ";
		}
  ...
  if(!running){
	  ...
	  setMotors(vLeft,vRight); 
		  ...
		  int followLineRed(){
			  ...
			  // if the error value is less than zero, & a white pixle is detected in front, turn left.
     	 if((error<0)&&((r<114) && (52<r)) && (error<0)){vRight = 60;} // If white Left And Front & right, turn right.
	    // If neither white Left And Front & right, turn arround left.
	    if(!((error<0)&&((r<114) && (52<r)) && (error<0))){
			  if ((y<114) && (52<y)){
		 vLeft = 40; 
		 vRight = 40;} else{
			vLeft = 60;}
		}
	 		if((error > 75)&&((r<114) && (52<r))){vLeft = 60;}

		//If the error value is greather then 75 and their is white directly in front, make the robot turn left.					
		if((error < 75)&&((r<114) && (52<r))){vRight = 60;}
